  ...
  return createTool({
    ...
    handler: async (params, { step: toolStep }) => {
      ...
      for (const fileId of fileIds) {
        ...
        if (!file) {
          return `Error: File with ID ${fileId} does not exist. Use listFiles tool to get the list of valid fileIDs.`;
        }
      ...
      try {
        return await toolStep?.run("delete-files", async () => {
          ...
          for (const file of filedToDelete) {
            await convex.mutation(api.system.deleteFile, {
              internalKey,
              fileId: file.id as Id<"files">,
            });
            results.push(
              `Successfully deleted ${file.type} "${file.name}" with ID ${file.id}`,
            );
          }
        ...
        return `Error deleting files: ${e instanceof Error ? e.message : "Unknown error"}`;
      }
    },
  });
};
