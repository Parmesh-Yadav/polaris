...
export const importGithubRepo = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    //sort the folders by depth so parents are created before the children
    ...
    //return the folder map so it can be used in subsequent steps
    //inngest serialized step results, so we use a plain object instead of a Map
    ...
    await step.run("create-files", async () => {
      for (const file of allFiles) {
        ...
        try {
          ...
          console.error(
            `Failed to import file: ${file.path} with sha: ${file.sha} from repo ${owner}/${repo}`,
            error,
          );
        }
      }
