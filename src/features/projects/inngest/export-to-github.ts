...
export const exportToGithub = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // wait for github to create the repo
    ...
    const treeItems = await step.run("create-blobs", async () => {
      ...
      for (const [path, file] of fileEntries) {
        ...
        if (file.content !== undefined) {
          ...
          // binary file - fetch content and encode as base64
    ...
    const { data: tree } = await step.run("create-tree", async () => {
      return await octokit.rest.git.createTree({
        ...
        repo: repo.name,
        tree: treeItems,
        base_tree: initialCommitSha,
      });
    });

    //create the commit with initial commit as the parent
    const { data: commit } = await step.run("create-commit", async () => {
      return await octokit.rest.git.createCommit({
        owner: user.login,
        repo: repo.name,
        message: "Export project files from Polaris",
        tree: tree.sha,
